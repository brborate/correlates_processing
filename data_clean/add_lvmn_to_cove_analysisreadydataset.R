# this was used to make data P3001ModernaCOVEimmunemarkerdata_correlates_processed_v1.1_lvmn_added_Jan14_2022.csv for the lvmn manuscript
...
for (t in c("B","Day29","Day57")) {
    ...
    dat_proc[[t%.%"liveneutmn50"]]=x[match(dat_proc$Ptid, dat.1$Ptid)]
}

with(dat_proc[dat_proc$Trt==1 & dat_proc$ph2.D57,], table(!is.na(Bliveneutmn50), Wstratum))
#       Wstratum
#         13  14  15  16  17  18  27
#  FALSE   0   0   0   0   0   0   4
#  TRUE  171 143 188 168 146 189  32
with(dat_proc[dat_proc$Trt==1 & dat_proc$ph2.D57,], table(!is.na(Day29liveneutmn50), Wstratum))
#       Wstratum
#         13  14  15  16  17  18  27
#  FALSE   4   5   9   2   6  11   5
#  TRUE  167 138 179 166 140 178  31
with(dat_proc[dat_proc$Trt==1 & dat_proc$ph2.D57,], table(!is.na(Day57liveneutmn50), Wstratum))
#       Wstratum
#         13  14  15  16  17  18  27
#  FALSE   1   0   0   1   1   0   5
#  TRUE  170 143 188 167 145 189  31
  
with(dat_proc[dat_proc$Trt==1 & dat_proc$ph2.D57,], table(!is.na(Bliveneutmn50), Wstratum, SubcohortInd))
with(dat_proc[dat_proc$Trt==1 & dat_proc$ph2.D57,], table(!is.na(Day29liveneutmn50), Wstratum, SubcohortInd))
with(dat_proc[dat_proc$Trt==1 & dat_proc$ph2.D57,], table(!is.na(Day57liveneutmn50), Wstratum, SubcohortInd))

# 5 cases missing lv mn50
subset(dat_proc[dat_proc$Trt==1 & dat_proc$ph2.D57,], (is.na(Bliveneutmn50) | is.na(Day29liveneutmn50) | is.na(Day57liveneutmn50)) & Wstratum==27, select=c(Ptid, Bliveneutmn50, Day29liveneutmn50, Day57liveneutmn50))
