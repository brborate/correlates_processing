...
if(study_name == "ENSEMBLE" & attr(config, "config") == "janssen_pooled_real"){
    ...
    if(all.equal(old_processed, new_processed) == TRUE){
      ...
      if(assertthat::assert_that(
        ...
        message("No change in input data. Superlearner will not be run. Risk scores from earlier run have been appended to data_processed.csv!")
      }
    }else{
      message("There is change in input data. Superlearner should be run as new risk scores need to be generated!")
    }
}
