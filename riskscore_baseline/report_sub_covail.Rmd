...
if(Sys.getenv("TRIAL") == "vat08_combined"){
...
}else if(studyName_for_report == "COVAIL"){
  footnote1 <- paste0("1. Binary input variable/s ", paste(unlist(strsplit(dropped.riskVars.fewer.0s1s, ",")), collapse = ", "), " had <= 3 cases ")
  footnote1b <- paste0("in the variable = 1 or 0 subgroup and dropped from analysis.")
  eval.footnote1.table = TRUE
  eval.footnote1b.table = TRUE
}else{
...
if(studyName_for_report %in% c("COVE", "MockCOVE")){
  ...
  dayrauc = "Day 29"
  aucstart = "1 day"
} else if (studyName_for_report %in% c("COVAIL")){
  dayrscore = "D15"
  rscorestart = "7 days"
...
if(Sys.getenv("TRIAL") == "covail"){
  caption <- "Individual baseline variables in COVAIL trial that were used in the Superlearner model for developing risk score and making CV-predictions for occurrence of COVID-19."
} else {
...
if(studyName_for_report %in% c("COVAIL")){
  tab <- tab %>% 
    footnote(c(footnote1,
               footnote1b,
               footnote2,
...
if(studyName_for_report %in% c("ENSEMBLE")){
  rm(tab, footnote1, footnote2, footnote3)
} else if(studyName_for_report %in% c("COVAIL")){
  rm(tab, footnote1, footnote1b, footnote2, footnote3)
} else {
...
if(studyName_for_report == "COVAIL"){
    cases_placebo$Argument <- sub("placebo group", "per-protocol cohort", cases_placebo$Argument)
}
...
```{r, echo=FALSE, out.width="99%", out.height="120%", fig.cap= paste0("Cross-validated AUC (95\\% CI) of algorithms for predicting COVID-19 disease status starting ", rscorestart, " after ", dayrscore, ". CV-AUCs were computed using data from all per-protocol participants.")}
...
```{r, echo=FALSE, out.height="94%", fig.cap=paste0("CV-estimated predicted probabilities of COVID-19 disease starting ", rscorestart, " after ", dayrscore, " by case/control status for top 2 learners, SuperLearner and Discrete SL. CV-estimated predicted probabilities were computed using data from all per-protocol participants.")}
# if(Sys.getenv("TRIAL") == "covail"){
#   knitr::include_graphics(here::here("riskscore_baseline", "output", Sys.getenv("TRIAL"), "predProb_riskscore.png"))
# } else {
...
```{r, echo=FALSE, out.width="95%", out.height="95%", fig.cap=paste0("ROC curves based off CV-estimated predicted probabilities for the top 2 learners, Superlearner and Discrete SL. CV-estimated predicted probabilities were derived considering cases as COVID-19 disease endpoints starting ", rscorestart, " after ", dayrscore, " using data from all per-protocol participants.")}
# if(Sys.getenv("TRIAL") == "covail"){
#   knitr::include_graphics(here::here("riskscore_baseline", "output", Sys.getenv("TRIAL"), "ROCcurve_riskscore.png"))
# } else {
...
caption <- "Weights assigned by Superlearner using data from all per-protocol participants."
...
**Interpretation of SuperLearner model:**  
SuperLearner gave highest weight to the SL.gam learner with univariate_logistic_pval screen (input for the SL.gam learner were variables having a Wald test 2-sided p-value < 0.10).  
...
$\textbf{Interpretation of SL.gam model: }$     

Being African-American was associated with a lower odds of acquiring COVID-19 after Day 15 through Day 188 by approximately 44\%.

Having a longer period of time elapsed from last prior COVID vaccination until enrollment into the COVAIL study (greater than the median value) was associated with a higher odds of acquiring COVID-19 after Day 15 through Day 188 by approximately 30\%.  

Having two Pfizer followed by one Moderna vaccination prior to enrollment was associated with higher odds of acquiring COVID-19 after Day 15 through Day 188 by approximately 27\% compared to receiving two Janssen Ad26.COV2.S vaccinations prior to enrollment.

The other variables in the model, namely, Age65C, s(Age, 2), and s(pre.study.booster.until.studydose1.day, 2), have odds ratios close to 1 and seem to have less impact on the overall model.  
...
The Baseline Risk Score analysis for COVAIL trial is conducted in all per-protocol participants. The analysis considers the following as input:

1. data_processed.csv file (created by the data processing step)

2. Riskscorecohortflag (created by the data processing step): This flag identifies the subjects in per-protocol cohort that are to be considered for risk score analysis. So, all subjects with Riskscorecohortflag == 1 have a risk score.

3. Risk score variables (shown in Table 1)

4. Endpoint: The endpoint used is `r endpoint`

Tables 6, 7 and 8 (below) show the number of subjects by study arm and endpoint status in the data before applying the Riskscorecohortflag, after applying the Riskscorecohortflag and after conducting the risk score analysis, respectively. 
...
if(any(sapply(c("COVE", "ENSEMBLE"), grepl, studyName_for_report))){
  ...
  eval.footnote1.table = TRUE
}else if(any(sapply(c("COVAIL"), grepl, studyName_for_report))){
  new_colnames = c("Non-Cases", paste0("Post-", dayrscore, "-Cases"))
  footnote1 <- paste0("Post-Day ", dayrscore, " Cases: COVID-19 primary endpoints that occur at least ", rscorestart, " after the ", dayrscore, " visit.")
  eval.footnote1.table = TRUE
}else{
...
<!-- read.csv(here("riskscore_baseline", "output", Sys.getenv("TRIAL"), "vacc.cases.AUC.calc_post_riskScoreAnalysis.csv"), row.names = 1, -->
<!--          header = TRUE) %>% -->
