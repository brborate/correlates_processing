...
This baseline risk score report is based off analysis data located here: `r mapped_data`.
...
if(studyName_for_report %in% c("COVE", "MockCOVE")){
  ...
  day = "Day 29"
  start = "1 day"
} else if (studyName_for_report == "PREVENT19"){
  day = "Day 35"
  start = "1 day"
  AUCstart = "7 days"
...
if(any(sapply(c("COVE", "ENSEMBLE"), grepl, studyName_for_report))){
  ...
  footnote1 <- paste0("Post-Day ", day, " Cases: COVID-19 primary endpoints that occur at least ", start, " after the ", day, " visit.")
  eval.footnote1.table = TRUE
}else{
...
read.csv(here("riskscore_baseline", "output", Sys.getenv("TRIAL"), "vacc.cases.AUC.calc_post_riskScoreAnalysis.csv"), row.names = 1,
         header = TRUE) %>%
  mutate(Var1 = ifelse(Var1 == 0, "Controls", "Cases")) %>%
  ...
  kable_styling(latex_options = c("hold_position"),
                font_size = 9) %>%
  footnote(c(paste0("Post-Day ", day, " Cases: COVID-19 primary endpoints that occur at least ", AUCstart, " after the ", day, " visit.")),
             threeparttable = TRUE)
