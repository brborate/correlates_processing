...
load(here("riskscore_baseline", "output", "objects_for_running_SL.rda"))
source(here("riskscore_baseline", "code", "study_specific_functions.R"))

study_name_code <- studyName_for_report

### create footnotes ###
dropped.riskVars.fewer.0s1s <- read.table(here("riskscore_baseline", "output", "drop_riskVars_with_fewer_0s_or_1s.csv")) %>%
...
dropped.riskVars.high.NAs <- read.table(here("riskscore_baseline", "output", "drop_riskVars_with_high_total_missing_values.csv")) %>%
  separate(V1, c("droppedVars"), sep = " ") %>%
  .$droppedVars %>%
  paste(collapse=", ") 
...
vars.with.imputed.values <- read.table(here("riskscore_baseline", "output", "impute_missing_values.csv")) %>%
...
tab <- read.csv(here("riskscore_baseline", "output", "risk_vars.csv"), row.names = 1,
         header = TRUE) %>%
  kable(
    #format = output_type,
    booktabs = TRUE,
...
read.csv(here::here("riskscore_baseline", "output", "learner-screens.csv"),
...
knitr::include_graphics(here::here("riskscore_baseline", "figs", "risk_placebo_cvaucs.png"))
```
...
knitr::include_graphics(here::here("riskscore_baseline", "figs", "predProb_riskscore_plac.png"))
```
...
knitr::include_graphics(here::here("riskscore_baseline", "figs", "ROCcurve_riskscore_plac.png"))
```
...
read.csv(here("riskscore_baseline", "output", "SL_weights.csv"), row.names = 1,
         header = TRUE) %>%
  kable(
    booktabs = TRUE,
    linesep = "",
...
read.csv(here("riskscore_baseline", "output", "SL_all_models_with_predictors.csv"),
         row.names = 1, header = TRUE) %>%
...
knitr::include_graphics(here::here("riskscore_baseline", "figs", "predProb_riskscore_vacc_onlySL.png"))
```
...
knitr::include_graphics(here::here("riskscore_baseline", "figs", "ROCcurve_riskscore_vacc_onlySL.png"))
```
...
The code for Baseline Risk Score analysis considers the following:

1. data_processed.csv file (created by the data processing step)

2. Riskscorecohortflag (derived in the data processing step): This flag identifies the subjects from both arms (placebo and vaccine) that are to be considered for risk score analysis. So, all subjects with Riskscorecohortflag == 1 have a risk score.
...
The risk score analysis code appends two columns (risk_score and standardized_risk_score) to the data_processed.csv file and drops out subjects that do not have a risk score (thus keeps only subjects with Riskscorecohortflag == 1). 
...
read.csv(here("riskscore_baseline", "output", "cases_prior_to_applying_Riskscorecohortflag.csv"), row.names = 1,
         header = TRUE) %>%
...
read.csv(here("riskscore_baseline", "output", "cases_prior_riskScoreAnalysis.csv"), row.names = 1,
         header = TRUE) %>%
  tibble::rownames_to_column(var = "Study-Arm") %>%
  rename_with(~new_colnames, c("X0", "X1")) %>%
  kable(
